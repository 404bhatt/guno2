...
def djangoapi(request,pk):
    return JsonResponse({"pk":pk}, safe=False)#HttpResponse("account is not active   "+str(pk))
...
def join_room(request,username,host):
    ...
    if roomz!=[]:
        ...
        return JsonResponse({"text":"host is incorrect"})
...
def start_game(request,host):
    ...
    for player_index in plyrs:
        game_state[player_index] = {'turn':'false','guno':False,'name':player_index,'drawed':0,
                                         'hand_cards':load['shuffel_deck'][:2]}
        load['shuffel_deck']=load['shuffel_deck'][2:]
    try:
...
def leave_game(request,username,host):
    ...
    if len(game_state['players'])!=1:
        ...
        if global_v['turnflag']==0:
            ...
            print('\n\n----------',global_v['pt'],num_players,global_v['turn_num'])
            turn=game_state['players'][#
            mod(num_players+(global_v['pt']+(global_v['pt']-mod(global_v['turn_num'],num_players))),num_players)
            ]#
...
def myturn(request,responce):
    ...
    if action==['draw']:
        ...
        if action[1]=='+2':
            ...
            #print('\n\nturn=',global_v['turnflag'])
            game_state['current']=action
            num_index=0
            for tem_i in game_state['players']:
                if turn==tem_i:
                    break
                else:
                    num_index+=1

            global_v['pt']=num_index
